<template>
    <div class="k-pagination" v-if="pagination.hasOtherPages">
        <KSelect :items="pageSizes" v-model="pageSize" label="Page Size" />
        <div class="k-pagination-buttons">
            <a class="k-pagination-button" v-if="hasPrevious">
            ...
            <template v-for="page in pageRange" :key="page">
                ...
                <a @click="$emit('update:modelValue', page)" class="k-pagination-button k-pagination-button-border" :class="classObj(page)" v-else>
                    {{ page }}
                </a>
            </template>

            <a class="k-pagination-button" v-if="hasNext">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24">
                    <path d="M7.33 24l-2.83-2.829 9.339-9.175-9.339-9.167 2.83-2.829 12.17 11.996z" />
                </svg>
            </a>
        </div>
        <div>Displaying {{ pagination.startIndex }}-{{ pagination.endIndex }} of {{ pagination.count }} items.</div>
    </div>
    <!-- <div>{{ pagination }}</div> -->
</template>
